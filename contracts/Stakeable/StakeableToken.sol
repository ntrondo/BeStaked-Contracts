 ...
 */
abstract contract Context is ERC20, ERC20Mintable {
...
abstract contract GlobalsAndUtility is Context,IDailyUpdate {
    ...
    */
	...
	/* Modified from original 1575331200(2019-12-03T00:00:00Z) to 1609459200(2021-01-01T00:00:00Z) to avoid out-of-memory in remix.*/
    uint256 internal constant LAUNCH_TIME = 1609459200;
     ...
     */
    function dailyDataUpdate(uint256 beforeDay) external override
    {
     ...
     * @return list Fixed array of packed values uint256[] memory list
     */
    ...
    function _currentDay() internal virtual view returns (uint256)
    {
     ...
     * @return payout in Hearts, uint256 payout
     */
 ...
 contract StakeableToken is GlobalsAndUtility, IStakeable {
    constructor()        
    {
     ...
     * @param stakerAddr Address of staker
     */
    function stakeCount(address stakerAddr) external override view
        returns (uint256)
    {
    ...
    function _stakeUnlock(GlobalsCache memory g, StakeCache memory st) private
        pure
    {
...
contract StakeableTokenMock is StakeableToken{
    // function getStakeId(uint256 stakeIndex) external view returns(uint40){     
    //     uint40 stakeId = 0;
    //     StakeStore[] memory stakes = stakeLists[address(msg.sender)];
    //     require(stakes.length > 0, "no stakes");
    //     require(stakeIndex < stakes.length, "HEXTest.getStakeId(unint256 stakeIndex): stakeindex < stakes.length");
    //     stakeId = stakes[stakeIndex].stakeId;
    //     return stakeId;
    // }
    uint256 private SKIP_DAYS = 0;
    function setCurrentDay(uint256 day)external{
        uint256 currentDay = _currentDay();        
        require(day >= currentDay);
        SKIP_DAYS += day - currentDay;        
    }
    function _currentDay() internal override view returns (uint256)
    {
        return _currentDayBase() + SKIP_DAYS;
    }
    function _currentDayBase() internal view returns (uint256)
    {
        return (block.timestamp - LAUNCH_TIME) / 1 days;
    }
}